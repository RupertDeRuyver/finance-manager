...
export class DataSource {
  ...
  private async prepareDB(): Promise<boolean> {
    log("Preparing database", 5);
    if (!this.db) {
      log("Database not initialized", 1);
      return false;
    }
    try {
  ...
  async getDB(): Promise<Kysely<Database> | null> {
    const result = await this.prepareDB();
    if (!result) {
      return null;
    }
    return this.db;
...
export async function updateTransactions(db: Kysely<Database>, gocardless_transactions: GocardlessTransaction[], userId: number): Promise<boolean> {
